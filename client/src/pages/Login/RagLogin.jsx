...
const RagLogin = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        if (data.role !== 'sag') {
          toast.error('Access denied. This portal is for SAG officials only.');
        ...
        getUserData();
        setRedirecting(true);
        setTimeout(() => navigate('/sag/applications'), 3000);
      } else {
      ...
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/2 p-8 sm:p-10">
          <div className="flex items-center gap-2 mb-6 cursor-pointer" onClick={() => navigate('/landing')}>
            ...
            <span className="font-bold text-orange-600">SSP</span>
          </div>

          <h2 className="text-2xl font-bold text-gray-900">Hey,</h2>
          <h2 className="text-2xl font-bold text-gray-900 mb-1">Welcome Back!</h2>
          <p className="text-gray-500 text-sm mb-6">
            Sign in to the SAG verification portal
          </p>
          ...
          <form onSubmit={onSubmitHandler} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  minLength={6}
                  className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition pr-10"
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
            >
          ...
          <p className="text-gray-400 text-center text-xs mt-6">
            SAG accounts are provisioned by the administrator.
          </p>
        </div>

        {/* Decorative Right Panel */}
        <div className="hidden md:block w-1/2 relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-orange-500 via-amber-400 to-yellow-300 opacity-80" />
          ...
          <div className="absolute bottom-10 left-10 w-48 h-48 bg-amber-300/30 rounded-full blur-2xl" />
          <div className="absolute top-10 right-10 w-36 h-36 bg-orange-400/30 rounded-full blur-2xl" />
          <div className="relative z-10 flex items-center justify-center h-full">
            <div className="text-center text-white p-8">
              <Shield className="w-16 h-16 mx-auto mb-4 drop-shadow-lg" />
              <h3 className="text-2xl font-bold mb-2">SAG Portal</h3>
              <p className="text-white/80 text-sm">Verify applications and manage scholarship approvals</p>
            </div>
          </div>
        </div>
      </div>
